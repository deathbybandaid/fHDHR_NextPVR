...
class fHDHRdb(object):
    ...
    def adjust_program_value(self, program, key, value, namespace='default'):
        ...
        try:
            ...
            # DNE - Insert
                ...
                new_programvalue = ProgramValues(program=program, namespace=namespace, key=key, value=float(value))
                session.add(new_programvalue)
                session.commit()
        except SQLAlchemyError:
            session.rollback()
    ...
    def adjust_program_list(self, program, key, entries, adjustmentdirection, namespace='default'):
        program = program.lower()
        if not isinstance(entries, list):
            entries = [entries]
        entries = json.dumps(entries, ensure_ascii=False)
            ...
            # DNE - Insert
                ...
                if adjustmentdirection == 'add':
                    ...
                    for entry in entries:
                        while entry in values:
                            values.remove(entry)
                new_programvalue = ProgramValues(program=program, namespace=namespace, key=key, value=values)
                session.add(new_programvalue)
                session.commit()
        except SQLAlchemyError:
            session.rollback()
    ...
    def adjust_cacheitem_value(self, cacheitem, key, value, namespace='default'):
        ...
        try:
            ...
            # DNE - Insert
                ...
                new_cacheitemvalue = CacheValues(cacheitem=cacheitem, namespace=namespace, key=key, value=float(value))
                session.add(new_cacheitemvalue)
                session.commit()
        except SQLAlchemyError:
            session.rollback()
    ...
    def adjust_cacheitem_list(self, cacheitem, key, entries, adjustmentdirection, namespace='default'):
        cacheitem = cacheitem.lower()
        if not isinstance(entries, list):
            entries = [entries]
        entries = json.dumps(entries, ensure_ascii=False)
            ...
            # DNE - Insert
                ...
                if adjustmentdirection == 'add':
                    ...
                    for entry in entries:
                        while entry in values:
                            values.remove(entry)
                new_cacheitemvalue = CacheValues(cacheitem=cacheitem, namespace=namespace, key=key, value=values)
                session.add(new_cacheitemvalue)
                session.commit()
        except SQLAlchemyError:
            session.rollback()
